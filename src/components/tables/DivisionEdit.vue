  ...
  methods:{
    onShow:function() {
      ...
      if (this.mode == 'add') {
        ...
        axios.get(`http://localhost:8000/api/division/${this.id}`, {
          headers: {
    ...
    submit () {
      ...
      if (this.mode == 'add') {
        axios.post(`http://localhost:8000/api/division`,  requestStr, {
          headers: {
            'Content-Type': 'application/json',
        ...
        // Если режим редактирования, то исползуем метод PUT с указанием текущего id записи
        axios.put(`http://localhost:8000/api/division/${this.id}`,  requestStr, {
          headers: {
            'Content-Type': 'application/json',
            'Authorization': 'JWT ' + this.getToken
          }
    ...
    closeDlg:function()
    {
      // Формирует событие closeDivisionEdit, которое отслеживает родительский компонент, вызвавший это окно
      this.$emit('closeDivisionEdit');
    }
  },
}
</script>
